...
- an optional-pack catalog that permits reviewed Matt and on-demand Ponytail Review profiles, installed or explicitly updated project-locally from pinned release tags with their resolved commits displayed; unreviewed catalog entries remain reference-only;
...
Optional packs follow the same rule. Installable packs are reviewed profiles from release tags whose resolved commits are recorded, through a pinned public installer and project target. Callers must name the target and profile or individual Skills; setup never requests an all-Skills or global install. Updates are explicit and replace only the selected managed project copies from the next reviewed release. Matt's own interactive setup remains a subsequent user-invoked action because it decides project issue-tracker, label, and document conventions. Anthropic remains cataloged as a reference-only source until its content and installation behavior are reviewed.
